...
public class TreeAlgorithms {
	...
	public static int getDiameter(BinaryTreeNode root) {
		...
		// This can be done in O(n), if height is passed & set as parameter instead of
		...
		return Utilities.max(getDiameter(root.getLeft()), getDiameter(root.getRight()),
	...
	private static void getPostOrderFromInOrderAndPreOrder(int[] inOrder, int[] preOrder, int inStart, int inEnd,
	...
	private static void getPostOrderFromInOrderAndPreOrder(int[] inOrder, int[] preOrder, int[] postOrder, int inStart,
			int inEnd, int preIndex, AtomicInteger postIndex) {
		if (inStart > inEnd) {
			return;
		}
		...
		getPostOrderFromInOrderAndPreOrder(inOrder, preOrder, postOrder, inIndex + 1, inEnd,
				preIndex + (inIndex - inStart) + 1, postIndex);
	...
	public static boolean isSubTree(BinaryTreeNode node1, BinaryTreeNode node2) {
		...
		return isSubList(inOrderNode1, inOrderNode2) && isSubList(preOrderNode1, preOrderNode2);
	}
	...
	public static boolean isSubTreeRecursive(BinaryTreeNode node1, BinaryTreeNode node2) {
		...
		return isSubTreeRecursive(node1.getLeft(), node2) || isSubTreeRecursive(node1.getRight(), node2);
	}
	...
	public static boolean areIdentical(BinaryTreeNode node1, BinaryTreeNode node2) {
		...
		return areIdentical(node1.getLeft(), node2.getLeft()) && areIdentical(node1.getRight(), node2.getRight());
	}
	...
	public static Pair<Integer, Integer> findFirstUnmatchedLeaves(BinaryTreeNode root1, BinaryTreeNode root2) {
		// An alternate approach is to do in-order traversal, add leaves to 2 arrays &
		// then compare them
	...
	private static void sumOfLongestRootToLeafPath(BinaryTreeNode node, int length, int sum, AtomicInteger maxLength,
			AtomicInteger maxSum) {
		if (node == null) {
			if (length > maxLength.intValue()) {
				maxLength.set(length);
				maxSum.set(sum);
			} else if (length == maxLength.intValue() && maxSum.intValue() < sum) {
				maxSum.set(sum);
			}
	...
	private static void constructStoreForAlternateLevelsOfPerfectBinaryTree(BTNode<Character> root,
			List<Character> store, int level) {
		if (root == null) {
			return;
		}
	...
	private static void constructTreeFromStoreForAlternateLevelsOfPerfectBinaryTree(BTNode<Character> root,
			List<Character> store, int level, AtomicInteger index) {
		if (root == null) {
			return;
		}
	...
	private static int getLargestSubtreeSum(BinaryTreeNode root, AtomicInteger maxSubtreeSum) {
		...
		int sum = root.getData() + getLargestSubtreeSum(root.getLeft(), maxSubtreeSum)
	...
	public static int lengthOfRootToLeafPathWithMostDistinctNodes(BinaryTreeNode root,
			Map<Integer, Integer> visitedNodes) {
		if (root == null) {
		...
		int maxLength = Math.max(lengthOfRootToLeafPathWithMostDistinctNodes(root.getLeft(), visitedNodes),
	...
	public static List<List<Integer>> getAllKSumPaths(BinaryTreeNode root, int k) {
		// Note: A path can start from any node and end at any node and must be downward
		// only, i.e. they need not be root node and leaf node.
	...
	public static void constructKSumPaths(BinaryTreeNode root, int k, List<List<Integer>> kSumPaths,
			List<Integer> path) {
		if (root == null) {
			return;
		}
	...
	private static BinaryTreeNode getKthAncestor(BinaryTreeNode node, int nodeData, AtomicInteger k,
			AtomicBoolean ancestorFound) {
		if (node == null) {
			return null;
		}
	...
	private static void constructDiagonalTraversal(BinaryTreeNode root, int diagonal,
	...
	public static boolean isBalanced(BinaryTreeNode root, AtomicInteger height) {
		...
		return Math.abs(leftHeight.intValue() - rightHeight.intValue()) <= 1 && leftBalanced && rightBalanced;
	...
	public static boolean isBalancedA2(BinaryTreeNode root) {
		...
		return Math.abs(leftHeight - rightHeight) <= 1 && isBalancedA2(root.getLeft()) && isBalancedA2(root.getRight());
	...
	public static void getDeepestNode(BinaryTreeNode root, int level, AtomicInteger deepestLevel,
			AtomicInteger deepestNode) {
		if (root == null) {
			return;
		}
	...
	private static boolean isPerfect(BinaryTreeNode root, int level, int height) {
		...
		return isPerfect(root.getLeft(), level + 1, height) && isPerfect(root.getRight(), level + 1, height);
	}
	...
	private static int longestConsecutiveIncreasingPath(BinaryTreeNode root, int previousValue, int previousLength) {
		...
		return Utilities.max(longestConsecutiveIncreasingPath(root.getLeft(), currentValue, currentLength),
				longestConsecutiveIncreasingPath(root.getRight(), currentValue, currentLength), previousLength);
	}

	public static void visitInOrderWithoutRecursionOrStack(BinaryTreeNode root) {
		visitInOrderWithoutRecursionOrStack(root, n -> System.out.print(n.getData() + " "));
	}

	public static void visitInOrderWithoutRecursionOrStack(BinaryTreeNode root, Consumer<BinaryTreeNode> visit) {
		if (root == null) {
			return;
		}
		...
		while (node != null) {
			...
			if(node.getRight() != null) {
				...
				// If node is the right child of its parent, switch to parent's parent till this does not hold.
				// Note: parent has been visited already if node is the right child.
				...
				// Once more to get the next node
				node = node.getParent();
			}
		}
	}
