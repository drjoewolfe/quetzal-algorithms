...
public class MinimizeHammingDistanceAfterSwapOperations {
    class Solution {
        public int minimumHammingDistance(int[] source, int[] target, int[][] allowedSwaps) {
            ...
            Map<Integer, Map<Integer, Integer>> partitionMap = new HashMap<>();
            for (int i = 0; i < n; i++) {
                int val = source[i];
                int ir = ds.find(i);

                if (!partitionMap.containsKey(ir)) {
                    partitionMap.put(ir, new HashMap<>());
                }

                var frequencyMap = partitionMap.get(ir);
